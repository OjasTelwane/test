...
const TestComponent = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      async function fetchData(testId) {
        const ql = await TestQuestionDataService.getAllTestQuestions(
          testId,
          sort,
          0,
          50
      ...
      const socket = io.connect(environment.settings.apiURL, {
        reconnect: true
      });
      if (socket) {
  ...
  const findAndReplaceQuestion = (updatedQuestion) => {
    ...
    const newQuestions = questions.map(
      (q) => updateQuestions.find((uq) => uq.id === q.id) || q
    );
  ...
  const updateQuestion = async (updatedTestQuestion, finish = false) => {
    ...
    const retQuestion = await TestQuestionDataService.updateTestQuestion(
      updatedTestQuestion.id,
      updatedTestQuestion
    );
  ...
  const finishStep = async (e) => {
    if (questions !== undefined) {
      ...
      await updateQuestion(updatedTestQuestion, true);
      handleEndTest(e);
    }
  };
  ...
  const handleResponse = async (e) => {
    ...
    await setQuestions((prevState) => {
      const newQuestions = prevState.map((item) =>
        item.id === option
          ? { ...item, selected_option_id: selectedOption }
          : { ...item }
      );
      return newQuestions;
    });
  ...
  const handleEndTest = async (e) => {
    console.log('handleEndTest called==>');

    const endTime = Date.now();
    var newEndTime = convertDateTimeToServer(endTime);
    await TestDataService.endTest(id, newEndTime);
    handle.exit();
    history.goBack();
  };
